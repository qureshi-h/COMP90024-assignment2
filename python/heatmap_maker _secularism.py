...
regionTweets = requests.get("http://admin:admin@172.26.133.72:5984/tweets/_design/CountSpecs/_view/RegionTweetDisplay?key=\"secularism\"")
regionTweetsJson = json.loads(regionTweets.text)

regionAurinData = requests.get("http://admin:admin@172.26.133.72:5984/lga_profiles/_design/Stats/_view/Secularism")
...
df["Most spoken languages (excluding english)"] = ""
df["1) "] = ""
df["2) "] = ""
df["3) "] = ""
df["4) "] = ""
...
for index, row in df.iterrows():
    for regions in regionAurinJson["rows"]:
        if row["LGA_NAME"]==regions["key"]:
            df.at[index,'1) '] = regions["value"]["1)"]
            df.at[index,'2) '] = regions["value"]["2)"]
            df.at[index,'3) '] = regions["value"]["3)"]
            df.at[index,'4) '] = regions["value"]["4)"]
            df.at[index,'5) '] = regions["value"]["5)"]
...
for i in range(len(df)):
    ...
    feature = folium.features.GeoJson(
        ...
        tooltip=folium.features.GeoJsonTooltip(
            fields=[
                'Most spoken languages (excluding english)',
                '1) ',
                '2) ',
                '3) ',
                '4) ',
...
heat_map.save('plots/secularismHeatMap.html')
print("plots/secularismHeatMap.html", end="")